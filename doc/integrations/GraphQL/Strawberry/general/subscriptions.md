...
在 GraphQL 中，您可以使用订阅（Subscriptions）从服务器传输数据。要在 Strawberry 上启用这个功能，你的服务器必须支持 ASGI 和 websockets，或者使用 AIOHTTP 集成。
...
与查询和变更一样，订阅在类中定义并传递给 Schema 函数。在这里，创建了基本的计数函数，从 0 计数到每个循环迭代之间的目标睡眠。

```{note}
`count` 的返回类型是 `AsyncGenerator`，其中第一个泛型参数是响应的实际类型，在大多数情况下，第二个参数应该保留为 `None` （关于Generator 类型的更多信息在[这里](https://docs.python.org/3/library/typing.html#typing.AsyncGenerator)）。
```

将发送以下 GraphQL 文档到服务器来订阅这个数据流：
...
这是非常简短的例子，说明了什么是可能的。与查询和变更一样，订阅可以返回任何 GraphQL 类型，而不仅仅是这里演示的标量。
